...
def main():
    ...
    if os.name != "nt":
        ...
        if os.path.exists(bad_cache):
            print(f"[*] Wiping old HF cache at {bad_cache} to force download of missing Vision configs...")
    ...
    model, _ = FastLanguageModel.from_pretrained(
        ...
        max_seq_length=1500,
        dtype=torch.bfloat16,  # Unsloth mathematically stabilizes bfloat16 to prevent NaN overflow!
        load_in_4bit=True,
        token=hf_token,
    )
